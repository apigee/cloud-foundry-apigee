...
function createProxy (bindReq, callback) {
  ...
  var mangledName = bindReq.proxyname || template(bindReq.configuration.get('APIGEE_PROXY_NAME_TEMPLATE'), {
    route: bindReq.bind_resource.route
      || bindReq.micro_coresident.target_app_route
      || bindReq.micro_c2c.target_app_route
      || bindReq.portal.target_app_name
  })
  ...
  } else if (Object.getOwnPropertyNames(bindReq.portal).length > 0) {
    var backendAppName = bindReq.portal.target_app_route.split(".")[0]

    mangledName = 'edgemicro_' + bindReq.portal.target_app_route
    union = Object.assign({
      domain: bindReq.configuration.get('APIGEE_PROXY_DOMAIN'),
      proxyname: mangledName,
      basepath: '/' + backendAppName,
      ...
      getVirtualHosts(proxyData, function (err, data) {
        if (err) {
          ...
          if(Object.getOwnPropertyNames(proxyData.micro_coresident).length > 0){
            ...
            dummyTargetUrl = proxyData.protocol + "://" + proxyData.micro_c2c.target_app_internal_route + ":" + proxyData.micro_c2c.target_app_port
            // dummyTargetUrl = proxyData.protocol + "://" + proxyData.bind_resource.route + ":" + proxyData.micro_c2c.target_app_port
          } else if (Object.getOwnPropertyNames(proxyData.portal).length > 0) {
            const protocol = proxyData.protocol ? proxyData.protocol : 'http'
            const port = proxyData.portal.port ? proxyData.portal.port : ''
            dummyTargetUrl = `${protocol}://${proxyData.portal.target_app_route}.${config.cf.appsBaseDomain}${port ? ':'+port : ''}`
          } else {
